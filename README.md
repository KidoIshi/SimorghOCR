...
アプリケーションウィンドウが開き、テキストを抽出したい画像またはPDFファイルをアップロードできます。OCRエンジンと言語（該当する場合）を選択し、「ファイルをアップロード」をクリックして進みます。OCR処理が完了すると、抽出されたテキストがアプリケーションウィンドウに表示され、Wordドキュメントとして保存するオプションが提供されます。
